...
class mainParser(IWrapper):
	...
	def process_main(self, parsed) -> main:
		rec = main()
		rec.size = self.backend.getSubTreeText(parsed.size)
		rec.typed = self.process_type_specific(parsed.typed)
		return rec

	def process_gearbox_specifier(self, parsed) -> gearbox_specifier:
		rec = gearbox_specifier()
		rec.ratio = self.backend.getSubTreeText(parsed.ratio)
		return rec
	...
	def process_three_phase_t(self, parsed) -> three_phase_t:
		rec = three_phase_t()
		rec.type_marker = self.backend.getSubTreeText(parsed.type_marker)
		rec.motor_body_length = self.backend.getSubTreeText(parsed.motor_body_length)
		rec.suffix = self.process_three_phase_suffix_opt_our(getattr(parsed, "suffix", None))
		return rec
	...
	def process_three_phase_suffix_t(self, parsed) -> three_phase_suffix_t:
		...
		rec.current_per_phase_over_ten = self.backend.getSubTreeText(parsed.current_per_phase_over_ten)
		rec.leads = self.backend.getSubTreeText(parsed.leads)
		rec.shafts_count = self.process_shafts_count_marker_opt_our(getattr(parsed, "shafts_count", None))
		return rec

	def process_stepper_suffix_t(self, parsed) -> stepper_suffix_t:
		rec = stepper_suffix_t()
		rec.suffix_body = self.process_suffix_with_or_without_tail(parsed.suffix_body)
		return rec
	...
	def process_suffix_with_current(self, parsed) -> suffix_with_current:
		...
		rec.current = self.backend.getSubTreeText(parsed.current)
		rec.tail = self.process_stepper_suffix_tail(parsed.tail)
		return rec
	...
	def process_shafts_count_and_step_rate_any_order_1_t(self, parsed) -> shafts_count_and_step_rate_any_order_1_t:
		...
		rec.shafts_count = self.process_shafts_count_marker_opt_our(getattr(parsed, "shafts_count", None))
		rec.step_rate = self.process_double_step_rate_marker(getattr(parsed, "step_rate", None))
		return rec
	...
	def process_stepper_suffix_tail(self, parsed) -> stepper_suffix_tail:
		...
		rec.variant = self.backend.getSubTreeText(parsed.variant)
		rec.shafts_count_and_step_rate_any_order = self.process_shafts_count_and_step_rate_any_order_t(parsed.shafts_count_and_step_rate_any_order)
		rec.encoder = self.process_encoder_presence_marker(getattr(parsed, "encoder", None))
		rec.unknown_F = self.process_unknown_marker_F(getattr(parsed, "unknown_F", None))
		rec.key_way = self.process_keyway_marker(getattr(parsed, "key_way", None))
		rec.screwed_shaft = self.process_screwed_shaft_marker(getattr(parsed, "screwed_shaft", None))
		rec.gearbox = self.process_gearbox_specifier_opt_our(getattr(parsed, "gearbox", None))
		rec.hollow = self.process_hollow_specifier_opt_our(getattr(parsed, "hollow", None))
		return rec
	...
	def process_stepper_t(self, parsed) -> stepper_t:
		rec = stepper_t()
		rec.type_marker = self.process_stepper_marker(parsed.type_marker)
		rec.stepper_modifiers = self.process_stepper_modifiers_opt_our(getattr(parsed, "stepper_modifiers", None))
		rec.motor_body_length = self.backend.getSubTreeText(parsed.motor_body_length)
		rec.suffix = self.process_stepper_suffix_t(parsed.suffix)
		return rec
	...
	def process_stepper_marker(self, parsed) -> stepper_marker:
		rec = stepper_marker()
		rec.series = self.process_stepper_series_opt_our(getattr(parsed, "series", None))
		return rec
	...
	def process_brushless_marker(self, parsed) -> brushless_marker:
		...
		rec.is_price_performance = self.backend.getSubTreeText(parsed.is_price_performance)
		rec.is_round = self.backend.getSubTreeText(parsed.is_round)
		rec.is_flat = self.backend.getSubTreeText(parsed.is_flat)
		rec.is_square = self.backend.getSubTreeText(parsed.is_square)
		rec.is_unknown_C = self.backend.getSubTreeText(parsed.is_unknown_C)
		return rec

	def process_execution_configuration_t(self, parsed) -> execution_configuration_t:
		rec = execution_configuration_t()
		rec.number = self.backend.getSubTreeText(parsed.number)
		rec.unit = self.backend.getSubTreeText(parsed.unit)
		return rec
	...
	def process_brushless_modifier_poles_torque(self, parsed) -> brushless_modifier_poles_torque:
		...
		rec.poles_or_other = self.backend.getSubTreeText(parsed.poles_or_other)
		rec.variant = self.backend.getSubTreeText(parsed.variant)
		return rec
	...
	def process_brushless_modifier_length_electronics_electrics(self, parsed) -> brushless_modifier_length_electronics_electrics:
		...
		rec.motor_body_length = self.backend.getSubTreeText(parsed.motor_body_length)
		rec.tail = self.process_brushless_tail_opt_our(getattr(parsed, "tail", None))
		return rec
	...
	def process_brushless_t(self, parsed) -> brushless_t:
		rec = brushless_t()
		rec.type_marker = self.process_brushless_marker(parsed.type_marker)
		rec.modifier = self.process_brushless_modifier_variants(parsed.modifier)
		return rec
	...
	def process_brushless_electronic_tail(self, parsed) -> brushless_electronic_tail:
		...
		rec.has_electronics = self.backend.getSubTreeText(parsed.has_electronics)
		rec.cfg = self.process_execution_configuration_opt_our(getattr(parsed, "cfg", None))
		return rec
